...
class ProcessController {
  ...
  tableMarketGroupByCep: {
    [x: string]: { weightInitial: string; weightFinal: string }[];
  };
  ...
  constructor(public tableSystem: Table, public tableMarket: Table) {
  ...
  private createTableTotalSystem() {
    ...
    cepOfGroupTableSystem.forEach(([cepInitialSystem, cepFinalSystem], i) => {
      ...
      const groupRangeCepSystem = this.getGroupSystemByRangeCep(
        cepInitialSystemInNumber + "-" + cepFinalSystemInNumber
      );
      ...
      for (let j = 0; j < cepOfGroupTableMarket.length; j++) {
        ...
        if (
          cepInitialMarketInNumber <= cepInitialSystemInNumber &&
        ...
        if (
          cepInitialMarketInNumber <= cepFinalSystemInNumber &&
      ...
      if (indexInitial >= 0 || indexFinal >= 0) {
        for (
          let k = indexInitial >= 0 ? indexInitial : indexFinal;
          k <= (indexFinal >= 0 ? indexFinal : indexInitial);
          k++
        )
          ranges.push(k);
      }
      ...
      groupRangeCepSystem.forEach(
        (
          {
            deadline,
            freight,
            weightFinal: weightFinalSystem,
            weightInitial: weightInitialSystem,
          },
          k
        ) => {
          ...
          weightsMarket.forEach(
            (
              {
                weightFinal: weightFinalMarket,
                weightInitial: weightInitialMarket,
              },
              l
            ) => {
              ...
              if (
                weightInitialMarketInNumber <= weightFinalSystemInNumber &&
          ...
          this.tableTotalSystem[indexLine][
            getParamsHeaderTotalSystem().cepInitial
          ] = cepInitialSystem;
          this.tableTotalSystem[indexLine][
            getParamsHeaderTotalSystem().cepFinal
          ] = cepFinalSystem;
          this.tableTotalSystem[indexLine][
            getParamsHeaderTotalSystem().deadline
          ] = `${deadline}`;
          this.tableTotalSystem[indexLine][
          ...
          this.tableTotalSystem[indexLine][
            getParamsHeaderTotalSystem().rangeWeight
          ] = indexFinal >= 0 ? `Faixa ${indexFinal + 1}` : "";
  ...
  private createTableTotalMarket() {
    ...
    for (let i = 0; i < cepOfGroupTableMarket.length; i++) {
      ...
      const biggestFreight =
        rangeCepTotalSystemOrderedByFreight[
          rangeCepTotalSystemOrderedByFreight.length - 1
        ];
      const biggestDeadline =
        rangeCepTotalSystemOrderedByDeadline[
          rangeCepTotalSystemOrderedByDeadline.length - 1
        ];

      const rangeCepGroupMarket = this.getGroupMarketByRangeCep(
        cepInitialMarket + "-" + cepFinalMarket
      );
      ...
      rangeCepGroupMarket.forEach(({ weightFinal, weightInitial }) => {
        ...
        this.tableTotalMarket[indexLine][
          getParamsHeaderTotalMarket().cepInitial
        ] = `${cepInitialMarket}`;
        this.tableTotalMarket[indexLine][
          getParamsHeaderTotalMarket().cepFinal
        ] = `${cepFinalMarket}`;
        this.tableTotalMarket[indexLine][
  ...
  private insertWeightInitialInTableSystem() {
    this.tableSystem.forEach((line, i) => {
      ...
      if (isNaN(lastWeightFinal)) {
        console.log(
          `Weight ${lastWeightFinal} of range cep ${
            line[getParamsHeaderSystem().cepInitial]
          } - ${line[getParamsHeaderSystem().cepFinal]} is not a number`
        );
        return;
      }
      ...
      if (!isFirstWeightOfCep) {
        line[getParamsHeaderSystem().weightInitial] = `${
          lastWeightFinal + (i != 1 ? 0.001 : 0)
        }`;
      } else {
  ...
  private insertWeightInitialInTableMarket() {
    this.tableMarket.forEach((line, i) => {
      ...
      if (isNaN(lastWeightFinal)) {
        console.log(
          `Weight ${lastWeightFinal} of range cep ${
            line[getParamsHeaderMarket().cepInitial]
          } - ${line[getParamsHeaderMarket().cepFinal]} is not a number`
        );
        return;
      }
      ...
      if (!isFirstWeightOfCep) {
        line[getParamsHeaderMarket().weightInitial] = `${
          lastWeightFinal + (i != 1 ? 0.001 : 0)
        }`;
      } else {
  ...
  private groupTableMarketByCep() {
    this.tableMarketGroupByCep = this.groupTableByCep(
      this.tableMarket,
      getParamsHeaderMarket()
    );
  }
  ...
  private groupTableSystemByCep() {
    this.tableSystem.forEach((line, i) => {
      ...
      if (
        typeof this.tableSystemGroupByCep[`${cepInitial}-${cepFinal}`] ==
        "undefined"
      ) {
  ...
  private getGroupMarketByIndex(index: number) {
    return Object.keys(this.tableMarketGroupByCep).map(
      (group) => this.tableMarketGroupByCep[group]
    )[index];
  }
